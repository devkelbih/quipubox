...
class TipoJabaViewModel extends DisposeSafeNotifier {
  final GetTiposJabaUseCase getItemsUseCase; final CreateTipoJabaUseCase createUseCase; final UpdateTipoJabaUseCase updateUseCase; final DeleteTipoJabaUseCase deleteUseCase; final NetworkChecker networkChecker;
  TipoJabaViewModel({required this.getItemsUseCase, required this.createUseCase, required this.updateUseCase, required this.deleteUseCase, required this.networkChecker});
  List<TipoJaba> items = []; bool isLoading = false; bool isSaving = false; bool isDeleting = false; String? errorMessage;
  ...
  Future<bool> save({int? id, required TipoJabaRequestModel request}) async { if (!await networkChecker.hasInternet()) { errorMessage = 'No hay conexión a internet. No se puede guardar.'; notifyListeners(); return false; } isSaving = true; errorMessage = null; notifyListeners(); try { if (id == null) { await createUseCase(request); } else { await updateUseCase(id, request: request); } await load(); return true; } on Object catch (e) { errorMessage = _clean(e); return false; } finally { isSaving = false; notifyListeners(); } }
