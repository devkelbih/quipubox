...
import '../../domain/usecases/delete_fruta.dart';
import '../../domain/usecases/get_frutas.dart';
import '../../domain/usecases/update_fruta.dart';
class FrutaViewModel extends DisposeSafeNotifier {
  final GetFrutasUseCase getItemsUseCase; final CreateFrutaUseCase createUseCase; final UpdateFrutaUseCase updateUseCase; final DeleteFrutaUseCase deleteUseCase; final NetworkChecker networkChecker;
  FrutaViewModel({required this.getItemsUseCase, required this.createUseCase, required this.updateUseCase, required this.deleteUseCase, required this.networkChecker});
  List<Fruta> items = []; bool isLoading = false; bool isSaving = false; bool isDeleting = false; String? errorMessage;
  ...
  Future<bool> save({int? id, required FrutaRequestModel request}) async { if (!await networkChecker.hasInternet()) { errorMessage = 'No hay conexión a internet. No se puede guardar.'; notifyListeners(); return false; } isSaving = true; errorMessage = null; notifyListeners(); try { if (id == null) { await createUseCase(request); } else { await updateUseCase(id, request: request); } await load(); return true; } on Object catch (e) { errorMessage = _clean(e); return false; } finally { isSaving = false; notifyListeners(); } }
