...
Future<void> _save() async {
  ...
  final request = UsuarioRequestModel(
    ...
    roleIds: widget.item == null
        ? [1] // temporal: luego esto saldrá del selector de roles
        : widget.item!.roles.map((e) => e.id).toList(),
  );

  final ok = await context.read<UsuarioViewModel>().save(
        id: widget.item?.id,
        request: request,
      );
  ...
  if (ok) {
    ...
    AppToast.show(
      context.read<UsuarioViewModel>().errorMessage ?? 'No se pudo guardar.',
      type: ToastType.error,
    );
  }
}
