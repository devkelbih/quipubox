...
import '../../domain/usecases/delete_calidad.dart';
import '../../domain/usecases/get_calidades.dart';
import '../../domain/usecases/update_calidad.dart';
class CalidadViewModel extends DisposeSafeNotifier {
  final GetCalidadesUseCase getItemsUseCase; final CreateCalidadUseCase createUseCase; final UpdateCalidadUseCase updateUseCase; final DeleteCalidadUseCase deleteUseCase; final NetworkChecker networkChecker;
  CalidadViewModel({required this.getItemsUseCase, required this.createUseCase, required this.updateUseCase, required this.deleteUseCase, required this.networkChecker});
  List<Calidad> items = []; bool isLoading = false; bool isSaving = false; bool isDeleting = false; String? errorMessage;
  ...
  Future<bool> save({int? id, required CalidadRequestModel request}) async { if (!await networkChecker.hasInternet()) { errorMessage = 'No hay conexión a internet. No se puede guardar.'; notifyListeners(); return false; } isSaving = true; errorMessage = null; notifyListeners(); try { if (id == null) { await createUseCase(request); } else { await updateUseCase(id, request: request); } await load(); return true; } on Object catch (e) { errorMessage = _clean(e); return false; } finally { isSaving = false; notifyListeners(); } }
