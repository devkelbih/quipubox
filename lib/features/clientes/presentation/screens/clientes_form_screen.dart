...
class _ClienteFormScreenState extends State<ClienteFormScreen> {
  ...
  Future<void> _save() async {
    ...
    final request = ClienteRequestModel(
      idEmpresa: widget.item == null
          ? context.read<AuthViewModel>().currentCompanyId
          : null,
    ...
    final ok = await context.read<ClienteViewModel>().save(
      id: widget.item?.id,
      request: request,
    );
    if (!mounted) return;
    if (ok) {
      ...
      AppToast.show(
        context.read<ClienteViewModel>().errorMessage ?? 'No se pudo guardar.',
