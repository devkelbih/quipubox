...
class LugarOperativoViewModel extends DisposeSafeNotifier {
  final GetLugaresOperativosUseCase getItemsUseCase; final CreateLugarOperativoUseCase createUseCase; final UpdateLugarOperativoUseCase updateUseCase; final DeleteLugarOperativoUseCase deleteUseCase; final NetworkChecker networkChecker;
  LugarOperativoViewModel({required this.getItemsUseCase, required this.createUseCase, required this.updateUseCase, required this.deleteUseCase, required this.networkChecker});
  List<LugarOperativo> items = []; bool isLoading = false; bool isSaving = false; bool isDeleting = false; String? errorMessage;
  ...
  Future<bool> save({int? id, required LugarOperativoRequestModel request}) async { if (!await networkChecker.hasInternet()) { errorMessage = 'No hay conexión a internet. No se puede guardar.'; notifyListeners(); return false; } isSaving = true; errorMessage = null; notifyListeners(); try { if (id == null) { await createUseCase(request); } else { await updateUseCase(id, request: request); } await load(); return true; } on Object catch (e) { errorMessage = _clean(e); return false; } finally { isSaving = false; notifyListeners(); } }
