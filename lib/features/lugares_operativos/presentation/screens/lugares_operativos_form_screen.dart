...
class LugarOperativoFormScreen extends StatefulWidget { final LugarOperativo? item; const LugarOperativoFormScreen({super.key, this.item}); @override State<LugarOperativoFormScreen> createState() => _LugarOperativoFormScreenState(); }
class _LugarOperativoFormScreenState extends State<LugarOperativoFormScreen> {
  ...
  @override void initState() { super.initState(); if (widget.item != null) { idSedeController.text = widget.item!.idSede.toString();
      ...
      direccionReferenciaController.text = widget.item!.direccionReferencia.toString();
  ...
  @override void dispose() { idSedeController.dispose();
    ...
    tipoLugarController.dispose(); super.dispose(); }
...
TextFormField(controller: direccionReferenciaController, keyboardType: TextInputType.text, decoration: const InputDecoration(labelText: 'Dirección o referencia'), ), const SizedBox(height: 12),
TextFormField(controller: observacionesController, keyboardType: TextInputType.text, decoration: const InputDecoration(labelText: 'Observaciones'), ), const SizedBox(height: 12),
TextFormField(controller: tipoLugarController, keyboardType: TextInputType.text, decoration: const InputDecoration(labelText: 'Tipo de lugar'), validator: (v) => v == null || v.trim().isEmpty ? 'Campo obligatorio' : null,), const SizedBox(height: 12), if (vm.errorMessage != null) Text(vm.errorMessage!, style: TextStyle(color: Theme.of(context).colorScheme.error))])))), actions: [TextButton(onPressed: vm.isSaving ? null : () => Navigator.of(context).pop(), child: const Text('Cancelar')), FilledButton(onPressed: vm.isSaving ? null : _save, child: vm.isSaving ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2)) : const Text('Guardar'))]); }
  Future<void> _save() async { if (!formKey.currentState!.validate()) return; final request = LugarOperativoRequestModel(idEmpresa: widget.item == null ? context.read<AuthViewModel>().currentCompanyId : null, idSede: int.tryParse(idSedeController.text.trim()) ?? 0, nombre: nombreController.text.trim(), direccionReferencia: direccionReferenciaController.text.trim().isEmpty ? null : direccionReferenciaController.text.trim(), observaciones: observacionesController.text.trim().isEmpty ? null : observacionesController.text.trim(), tipoLugar: tipoLugarController.text.trim()); final ok = await context.read<LugarOperativoViewModel>().save(id: widget.item?.id, request: request); if (!mounted) return; if (ok) { Navigator.of(context).pop(); AppToast.show('Guardado correctamente.', type: ToastType.success); } else { AppToast.show(context.read<LugarOperativoViewModel>().errorMessage ?? 'No se pudo guardar.', type: ToastType.error); } }
}
