...
import '../../domain/usecases/delete_variedad.dart';
import '../../domain/usecases/get_variedades.dart';
import '../../domain/usecases/update_variedad.dart';
class VariedadViewModel extends DisposeSafeNotifier {
  final GetVariedadesUseCase getItemsUseCase; final CreateVariedadUseCase createUseCase; final UpdateVariedadUseCase updateUseCase; final DeleteVariedadUseCase deleteUseCase; final NetworkChecker networkChecker;
  VariedadViewModel({required this.getItemsUseCase, required this.createUseCase, required this.updateUseCase, required this.deleteUseCase, required this.networkChecker});
  List<Variedad> items = []; bool isLoading = false; bool isSaving = false; bool isDeleting = false; String? errorMessage;
  ...
  Future<bool> save({int? id, required VariedadRequestModel request}) async { if (!await networkChecker.hasInternet()) { errorMessage = 'No hay conexión a internet. No se puede guardar.'; notifyListeners(); return false; } isSaving = true; errorMessage = null; notifyListeners(); try { if (id == null) { await createUseCase(request); } else { await updateUseCase(id, request: request); } await load(); return true; } on Object catch (e) { errorMessage = _clean(e); return false; } finally { isSaving = false; notifyListeners(); } }
